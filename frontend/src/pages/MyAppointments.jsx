...
const MyAppointments = () => {
  ...
  const getUsersAppointment=async (req,res)=>{
    try {
  ...
  const downloadReceipt = async (appointmentId) => {
    try {
        const response = await axios.post(
            backendUrl + "/api/user/download-receipt",
            { appointmentId },
            {
           ...
           appointments.map((item,index)=>(
              <div className='grid grid-col-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 border-b' key={index}>
                ...
                <div className='flex flex-col gap-2 justify-end'>
                  ...
                  {item.payment && <button onClick={() => downloadReceipt(item._id)}>Download Receipt</button>}
                 ...
                 {item.isCompleted && <button className='sm:min-w-48 py-2 border bordeer-green-500 rounded text-green-500'>Completed</button>}
                </div>
              </div>
            ))
          }
